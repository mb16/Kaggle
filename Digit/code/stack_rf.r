...
for(j in 1:Repeats) {
	...
	for(i in 1:CVFolds) {
		...
		#XXtest <- X[folds[[i]],-1]	
		print(Sys.time())
		# 150 seems about optimal for mtry
		RandF <- randomForest(x=XX, y=as.factor(YY), ntree=100, mtry=150, importance=FALSE, na.action=na.omit,keep.forest = TRUE) #keep.forest = FALSE helps memory
		print(Sys.time())
		...
		#print(colnames(XX))
		cNames <- colnames(XX)
		pred <-predict(RandF,data.frame(XX), type="vote", norm.votes=FALSE) #, se.fit= TRUE, type="vote", norm.votes=FALSE
	...
	write.table(testPredictions, file="temp\\testPredictions_matrix_rf.txt", row.names = FALSE, col.names = FALSE)  
	...
	write.table(trainPredictions, file="temp\\trainPredictions_matrix_rf.txt", row.names = FALSE, col.names = FALSE) #, row.names = FALSE, col.names = FALSE
