...
def run_stack(SEED, col, alpha):
    ...
    clf = Lasso(alpha=alpha) # found with tune_lasso.py
    ...
    while True:
        ...
        if len(sorted_coefs) < 60 :
            threshold = sorted_coefs[0]
    ...
    trainBase.to_csv("../models/" + str(clf)[:5] + dset + "_train_" + col + ".csv", index = False)
    
    
    gc.collect()
    test = pd.read_csv('../preprocess/pre_shuffled_test' + dset + '.csv')
    test = test.loc[:,columnsHighScore] 
    test.to_csv("../models/" + str(clf)[:5] + dset + "_test_" + col + ".csv", index = False)  
...
if __name__=="__main__":
    ...
    #colsTarget = ['Sand']   
    #alphas = [0.00961077966238,0.0356224789026,0.00621016941892,0.0148735210729,0.00961077966238]# for dataset 2 
    #alphas = [0.00961077966238,0.0853167852417,0.00961077966238,0.0148735210729,0.00961077966238]  
    alphas = [ 0.00280721620394,0.00489390091848,0.000699564215671,0.000699564215671,0.000923670857187]  
