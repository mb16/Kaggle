...
for(i in 1:CVFolds) {
  ...
  currScore = normalized_weighted_gini(YY, pred$fit, WW)
...
write.table(mat1[c("id", "target")], file=paste("..\\submission\\Blend_", format(Sys.time(), "%Y%m%d%H%M%S"), "_", (cumScore/CVFolds), "_GAM.csv", sep = ""), row.names = FALSE, col.names = TRUE, sep = ",")   
...
write.table(mat2[c("id", "target")], file=paste("..\\submission\\Target_Blend_", format(Sys.time(), "%Y%m%d%H%M%S"), "_", (cumScore/CVFolds), "_GAM.csv", sep = ""), row.names = FALSE, col.names = TRUE, sep = ",") #, row.names = FALSE, col.names = FALSE
