	...
	for line in f.readlines():
		...
		for item in sample:
			if ( item.replace('.','',1).replace('-','',1).replace('+','',1).isdigit() ) :
				try: # DATES AND HYPHENATED ZIP CODES CANNOT BE CONVERTED TO FLOAT, THE EXCEPTION WILL AS A STRING.
					if ( item == "1" or item == "0"):					
...
def write_line_file(file_path, data, filemode="a"): # filemode can be "a" for append
	f_out = open(file_path,filemode)
