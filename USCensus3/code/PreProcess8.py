...
def PreProcess4(N_Features):
	...
	trainBase = csv_io.read_data("PreProcessData/training_PreProcess8.csv", skipFirstLine = False, split = "\t")
	test = csv_io.read_data("PreProcessData/test_PreProcess8.csv", skipFirstLine = False, split = "\t")
	shutil.copy2("PreProcessData/DataClassList8.csv", "PreProcessData/DataClassList9.csv")	
	
	target = [x[0] for x in trainBase]
	train = [x[1:] for x in trainBase]
	
	DataClassList = csv_io.read_data("PreProcessData/DataClassList9.csv", False)
	...
	csv_io.write_delimited_file("PreProcessData/training_PreProcess9_" + str(NumFeatures) + ".csv", trainNew, delimiter="\t")		
	csv_io.write_delimited_file("PreProcessData/test_PreProcess9_" + str(NumFeatures) + ".csv", testNew, delimiter="\t")
