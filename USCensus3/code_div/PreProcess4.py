...
def PreProcess4():
	...
	trainBase = csv_io.read_data("PreProcessData/training_PreProcess3.csv", split="\t" ,skipFirstLine = False)
	...
	for n_neighbor in n_neighborsArr:
		for leaf_s in leaf_sizeArr:	
			...
			print "Save Train Values"
			csv_io.write_delimited_file("PreProcessData/training_PreProcess4.csv", trainBase, delimiter="\t")	
			trainBase = []
			
			print "Load test values"
			test = csv_io.read_data("PreProcessData/test_PreProcess3.csv", split="\t" ,skipFirstLine = False)
	...
	with open("PreProcessData/DataClassList4.csv", "a") as myfile:
		myfile.write("Lat-Long-Predictor\n")

	print "Writing Data"
		
	csv_io.write_delimited_file("PreProcessData/test_PreProcess4.csv", test, delimiter="\t")
	print "Done."	
