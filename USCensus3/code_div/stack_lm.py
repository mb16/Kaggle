...
def run_stack(SEED):
	...
	model = "DIV Long-Lat KNN5 " + str(NumFeatures) + " Features, " + Set

	print "Running GB, RF, ET stack."

	trainBase = csv_io.read_data("PreProcessData/training_PreProcess6_" + Set + str(NumFeatures) + ".csv", skipFirstLine = False, split = "\t")
	test = csv_io.read_data("PreProcessData/test_PreProcess6_" + Set + str(NumFeatures) + ".csv", skipFirstLine = False, split = "\t")
	...
	clfs = [
		GradientBoostingRegressor(loss='ls', learn_rate=0.05, subsample=0.5, max_depth=6, n_estimators=3000, random_state=166, min_samples_leaf=1),	
	...
	# LinearRegression(fit_intercept=True, normalize=False, copy_X=True),
	# Ridge(alpha=1.0, fit_intercept=True, normalize=False, copy_X=True, tol=0.001),
	# RandomForestClassifier(n_estimators=50, n_jobs=1, criterion='gini', random_state=5),
	# RandomForestClassifier(n_estimators=50, n_jobs=1, criterion='entropy', random_state=5),
	# RandomForestClassifier(n_estimators=75, n_jobs=1, criterion='gini', random_state=5),
	# RandomForestClassifier(n_estimators=75, n_jobs=1, criterion='entropy', random_state=5),
	# ExtraTreesClassifier(n_estimators=50, min_density=0.2, n_jobs=1, criterion='gini', bootstrap=True, random_state=5),
	# ExtraTreesClassifier(n_estimators=50, min_density=0.2, n_jobs=1, criterion='entropy', bootstrap=True, random_state=5)
	# ExtraTreesClassifier(n_estimators=75, min_density=0.2, n_jobs=1, criterion='gini', bootstrap=True, random_state=5),
	# ExtraTreesClassifier(n_estimators=75, min_density=0.2, n_jobs=1, criterion='entropy', bootstrap=True, random_state=5)
	# KNeighborsRegressor(n_neighbors=25, weights='uniform', algorithm='auto', leaf_size=30, warn_on_equidistant=False, p=2)
