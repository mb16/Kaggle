...
def PreProcess4():
	...
	trainBase = csv_io.read_data("PreProcessData/training_PreProcess2.csv", skipFirstLine = False, split = "\t")
	test = csv_io.read_data("PreProcessData/test_PreProcess2.csv", skipFirstLine = False, split = "\t")
	shutil.copy2("PreProcessData/DataClassList2.csv", "PreProcessData/DataClassList4Base.csv")	
	
	target = [x[0] for x in trainBase]
	train = [x[1:] for x in trainBase]
	
	DataClassList = csv_io.read_data("PreProcessData/DataClassList4Base.csv", False)
	...
	csv_io.write_delimited_file("PreProcessData/training_PreProcess4_Base.csv", trainNew, delimiter="\t")		
	csv_io.write_delimited_file("PreProcessData/test_PreProcess4_Base.csv", testNew, delimiter="\t")
