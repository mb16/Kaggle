...
def run_stack(SEED):
	...
	model = "40 Features KNN-NULLs"

	print "Running GB, RF, ET stack."

	trainBase = csv_io.read_data("PreProcessData/training_PreProcess5_Base.csv", skipFirstLine = False, split = "\t")
	test = csv_io.read_data("PreProcessData/test_PreProcess5_Base.csv", skipFirstLine = False, split = "\t")
	...
	clfs = [GradientBoostingRegressor(learn_rate=0.05, subsample=0.5, max_depth=10, n_estimators=2000, random_state=166)
			]		
	...
	for ExecutionIndex, clf in enumerate(clfs):
		...
		csv_io.write_delimited_file_single("../predictions_40/Stack_" + now.strftime("%Y%m%d%H%M%S") + "_" + str(avg) + "_" + str(clf)[:12] + ".csv", dataset_blend_test_set.mean(1))
		
		csv_io.write_delimited_file_single("../predictions_40/Target_Stack_" + now.strftime("%Y%m%d%H%M%S") + "_" + str(avg) + "_" + str(clf)[:12] + ".csv", dataset_blend_train[:,ExecutionIndex] )		
