...
def PreProcess2():
	...
	for TrainDatum in TrainDataNew:	
		for Feature in range(NumFeatures):
			if (MaxArray[Feature] - MinArray[Feature]) > 0.0 and MaxArray[Feature] != 1.0 and MinArray[Feature] != 0.0: # all remaining features are 0/1
	...
	csv_io.write_delimited_file("PreProcessData/training_PreProcess2_temp.csv", TrainDataNew)		
	csv_io.write_delimited_file("PreProcessData/test_PreProcess2_temp.csv", TestData)	
