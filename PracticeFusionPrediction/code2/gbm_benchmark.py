...
def main():
	...
	#EstLearnArray = [[160,0.5],[320,0.2],[640,0.1],[1280,0.05],[4000,0.01],[8000,0.005],[16000,0.001]]
	EstLearnArray = [[160,0.5]]
	
	for EstLearn in EstLearnArray:
		...
		for train_index, test_index in Folds:
			...
			gbm = GradientBoostingClassifier(loss='deviance', learn_rate=EstLearn[1], n_estimators=EstLearn[0], subsample=1.0, min_samples_split=1, min_samples_leaf=1, max_depth=3, init=None, random_state=456) # , max_features=None

			gbm.fit(train, target)
			prob = gbm.predict_proba(trainTest) 
			...
			print "EstLearn: ", EstLearn[0], EstLearn[1]
			print -probSum/len(prob)
	
 
			avg += 	(-probSum/len(prob))/NumFolds

			predicted_probs = gbm.predict_proba(test)  # was test
