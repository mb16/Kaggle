...
Xtrain <- as.data.frame(read.table("../train.csv",
                   header=TRUE,
                   sep= ","       
                   ))
Xtest <- as.data.frame(read.table("../test.csv",
                   header=TRUE,
                   sep= ","       
                   ))
...
for(j in 1:Repeats) {
	...
	for(i in 1:CVFolds) {
		...
		Xtemp$ACTION = class.ind(Xtemp$ACTION)
		nn <- nnet(ACTION ~ ., data=Xtemp, size = 40, MaxNWts = 4000, rang = 0.1,decay = 5e-4, maxit = 30)
		...
		XtempTarget <- as.data.frame(Xtrain[folds[[i]],1])
		#print(colnames(XX))
		#cNames <- colnames(XX)
		XXtest <- X[folds[[i]],-1]	
		...
		print(colnames(XtempTarget))
		#print(nrow(pred))
		write.table(pred, file="pred.txt", row.names = FALSE, col.names = FALSE)  
		
		print(pred)
		auc(XtempTarget , pred)
		quit()
