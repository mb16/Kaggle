...
def run_stack():
	...
	trainBase = csv_io.read_data("../TrainingSet/TrainingSet/ACT4_competition_training.csv", False)
	test = csv_io.read_data("../TestSet/TestSet/ACT4_competition_test.csv", False)
	...
	#clfs = [GradientBoostingRegressor(learn_rate=0.05, subsample=0.5, max_depth=6, n_estimators=50),
	#		GradientBoostingRegressor(learn_rate=0.02, subsample=0.2, max_depth=8, n_estimators=125),
	#		RandomForestRegressor(n_estimators=100, n_jobs=1, criterion='mse',compute_importances=True)
	#		]
	...
	for ExecutionIndex, clf in enumerate(clfs):
		...
		for train_index, test_index in Folds:
			...
			target = [x[1] for x in trainBaseTemp]
			train = [x[2:] for x in trainBaseTemp]
	
			testBaseTemp = [trainBase[i] for i in test_index]
			targetTest = [x[1] for x in testBaseTemp]
			trainTest = [x[2:] for x in testBaseTemp]
			...
			predicted_probs = clf.predict(testNew)  # was test						
			#print [x[1] for x in predicted_probs]
			predicted_list.append([predicted_probs]) #x[1] for x in 
